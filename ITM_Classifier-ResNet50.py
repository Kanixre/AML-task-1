...
class ITM_DataLoader():
    ...
    IMAGES_PATH = r"C:\Users\cpran\Desktop\tesst\flickr8k.dataset-cmp9137-item1\flickr8k-resised"
    train_data_file = r"C:\Users\cpran\Desktop\tesst\flickr8k.dataset-cmp9137-item1\flickr8k.TrainImages.txt"
    test_data_file = r"C:\Users\cpran\Desktop\tesst\flickr8k.dataset-cmp9137-item1\flickr8k.TestImages.txt"
    dev_data_file = r"C:\Users\cpran\Desktop\tesst\flickr8k.dataset-cmp9137-item1\flickr8k.DevImages.txt"
    sentence_embeddings_file = r"C:\Users\cpran\Desktop\tesst\flickr8k.dataset-cmp9137-item1\flickr8k.cmp9137.sentence_transformers.pkl"
    ...
    def process_input(self, img_path, dense_vector, text, label):
        ...
        features = {}
        features["input_1"] = img  # Changed from "image_input" to match the input name in the model
        features["text_embedding"] = dense_vector
        # 'caption' and 'file_name' are not used by the model, they are for your reference only
        features["caption"] = text
        features["file_name"] = img_path
    ...
    def print_data_samples(self, dataset):
        ...
        for features_batch, label_batch in dataset.take(1):
            for i in range(1):
                # Update the key used here to the new input name
                print(f'Image pixels: {features_batch["input_1"]}')  # Changed from "image_input" to "input_1"
                print(f'Sentence embeddings: {features_batch["text_embedding"]}')
                print(f'Caption: {features_batch["caption"].numpy()}')
                label = label_batch.numpy()[i]
                print(f'Label : {label}')
...
class ITM_Classifier(ITM_DataLoader):
    ...
    # # Resnet50 test
    # def create_vision_encoder(self, num_projection_layers, projection_dims, dropout_rate):
    #     base_model = ResNet50(weights='imagenet', include_top=False, input_tensor=Input(shape=self.IMAGE_SHAPE))
    #     x = base_model.output
    ...
    # Resnet50 test 2
    def create_vision_encoder(self, num_projection_layers, projection_dims, dropout_rate):
        # Name the input tensor to match with the dictionary keys during training
        img_input = Input(shape=self.IMAGE_SHAPE, name="input_1")  # Renamed from "image_input"
        ...
        # Set ResNet50 layers to non-trainable
        for layer in base_model.layers:
    ...
    #ResNet Test 2
    def create_text_encoder(self, num_projection_layers, projection_dims, dropout_rate):
        # Ensure input name matches with what the model expects in the data dictionary
        text_input = Input(shape=(self.SENTENCE_EMBEDDING_SHAPE,), name='text_embedding')
        outputs = self.project_embeddings(text_input, num_projection_layers, projection_dims, dropout_rate)
        return text_input, outputs
    ...
    def test_classifier_model(self):
        ...
        balanced_acc = balanced_accuracy_score(truths, predictions)
